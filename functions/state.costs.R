state.costs <- function(data, state, site_types){
  # Seelect state
    ...
    sim_summary$total_cost <- rowSums(sim_summary[, -1], na.rm = T) # Total cost [, -1] so as not to include the sim_no in total
    colnames(sim_summary)[16] <- paste0(state, "_total_cost")
    sim_summary <- sim_summary[, c("sim_no", paste0(state, "_total_cost"))]
    return(sim_summary)
}
