...
load.results.summary.old <- function(no.variables, locationSaveResults) {
  library(data.table)
  locationOfSummary <- paste(locationSaveResults, "/Summary/", sep="")
  ...
  for (i in 1:nrow(listFiles)) {
    try(load(file = paste(locationOfSummary,as.character(listFiles[i,c('files')]), sep = "")))
    no.per.state.wide[, as.character((ncol(no.per.state.wide)+1):(ncol(no.per.state.wide)+ncol(summaryStates.table))):=summaryStates.table]
    print(i)
  }
...
load.results.summary.old.no.ctrl <- function(no.variables, locationSaveResults) {
  library(data.table)
  locationOfSummary <- paste(locationSaveResults, "/Summary/", sep="")
